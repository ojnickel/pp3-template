 ...
 * 20150519
 * In die Methode "getData" wurde ein Filter fuer spezifische Unternehmen eingefuegt, 
 * der nicht zum Unternehmen gehoerende Suchergebnisse aus der Liste der Suchergebnisse entfernt.
 */
...
class SearchModelSearch extends JModelLegacy
{
	...
	public function getData()
	{
		...
		if (empty($this->_data))
		{
			...
			// Josef Mueller OWS, Filter für spezifische Unternehmen
			$server_name = $_SERVER['SERVER_NAME'];
			$unternehmen = stristr(strtolower($server_name), "-pfennigparade.de", true);
			if (!$unternehmen) {
				$unternehmen = stristr(strtolower($_SERVER['HTTP_REFERER']), "-suche", true);
				$unternehmen = substr($unternehmen, strripos($unternehmen, '/') + 1);
			}
			if ($unternehmen) {
				...
				for ($i = count($rows) - 1; $i >= 0; $i--) {
					$row = $rows[$i];
					$section = $row->section;
					if (false !== stripos($section, "Kontakte")) {
							array_splice($rows, $i, 1);
					} else if (0 != strcasecmp($usection, $section)) {
						array_splice($rows, $i, 1);
					}
				}
			}
