...
require_once JPATH_SITE . '/modules/mod_jevents_latest/tmpl/default/latest.php';
...
class AlternativeModLatestView extends DefaultModLatestView {
	...
	function displayLatestEvents() {
		...
		if ( isset( $this->eventsByRelDay ) && count( $this->eventsByRelDay ) ) {
			$content .= $this->modparams->get( 'modlatest_templatetop' ) || $this->modparams->get( 'modlatest_templatebottom' ) ? $this->modparams->get( 'modlatest_templatetop' ) : '<div id="jevents_body"><table align="center" width="98%" cellspacing="0" cellpadding="5" class="ev_table">
				<thead>
					<tr style="height: 3em">
			...
			$content .= $this->modparams->get( 'modlatest_templatebottom' ) || $this->modparams->get( 'modlatest_templatetop' ) ? $this->modparams->get( 'modlatest_templatebottom' ) : "</tbody></table>\n";
			$js       = new JSite();
			$menu     = $js->getMenu();
			$link     = $menu->getItem( 1142 )->route;
			$content .= '<a href="/' . $link . '" class="readmore">Alle Veranstaltungen</a></div>';
		} elseif ( $this->modparams->get( 'modlatest_NoEvents', 1 ) ) {
